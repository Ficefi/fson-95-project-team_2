import{a as h,P as z}from"./vendor-b592f4c5.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))a(o);new MutationObserver(o=>{for(const c of o)if(c.type==="childList")for(const i of c.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function s(o){const c={};return o.integrity&&(c.integrity=o.integrity),o.referrerpolicy&&(c.referrerPolicy=o.referrerpolicy),o.crossorigin==="use-credentials"?c.credentials="include":o.crossorigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function a(o){if(o.ep)return;o.ep=!0;const c=s(o);fetch(o.href,c)}})();function j(e){const t=JSON.parse(localStorage.getItem("cart"))||[],s=Array(t).flat(1/0);s.push(e),t.includes(e)||localStorage.setItem("cart",JSON.stringify(s))}function _(e){const t=JSON.parse(localStorage.getItem("cart")),s=Array(t).flat(1/0),a=s.indexOf(e);s.splice(a,1),localStorage.setItem("cart",JSON.stringify(s))}function b(e){const t=JSON.parse(localStorage.getItem("cart"));return Array(t).flat(1/0).some(a=>a===e)}const G="/fson-95-project-team_2/assets/fruitCart@2x-44c39800.png",R="/fson-95-project-team_2/assets/check-mark-35f2f86f.png",S=document.querySelector(".qty-card"),Y=JSON.parse(localStorage.getItem("cart"))||[];let K=Y.length;function $(e){S.textContent=String(e)}$(K);const n="/fson-95-project-team_2/assets/icons-b3cb9da3.svg",g=document.querySelector(".js-modal"),U=document.querySelector(".modal-content"),m=document.querySelector(".js-modal-subscription"),O=document.querySelector(".subscription-modal-content"),f=document.querySelector(".js-modal-order"),Q=document.querySelector(".subscription-order-content");async function E(e){g.classList.toggle("is-hidden"),await ce(e).then(t=>{const{img:s,name:a,desc:o,category:c,size:i,popularity:d,price:l,is10PercentOff:u}=t,w=o.split("");if(U.innerHTML=`<div class='image-wrap'>
  ...
  `,console.log(u),u===!0){const r=document.querySelector(".discount-icon-modal");console.log(r),r.classList.remove("is-hidden")}w.length>300?document.querySelector(".modal-descr").classList.add("modal-descr-fixed"):document.querySelector(".modal-descr").classList.remove("modal-descr-fixed"),window.addEventListener("keydown",C);function C(r){r.key==="Escape"&&g.classList.add("is-hidden")}window.onclick=function(r){r.target===g&&g.classList.add("is-hidden")};const I=document.querySelector(".modal-close-btn");I.addEventListener("click",N);function N(r){r.currentTarget===I&&g.classList.add("is-hidden")}const p=document.querySelector(".modal-cart-btn");p.addEventListener("click",A),b(e)&&(p.childNodes[0].data="Remove from",p.style.minWidth="175px");function A(){b(e)?(_(e),p.childNodes[0].data="Add to",p.style.minWidth="130px",$(S.outerText-1)):($(Number(S.outerText)+Number(1)),j(e),p.childNodes[0].data="Remove from",p.style.minWidth="175px")}g.classList.contains("is-hidden")&&(removeEventListener("click",N),removeEventListener("click",A),removeEventListener("keydown",C))})}async function V(e){m.classList.toggle("is-hidden"),await ne(e).then(o=>{O.innerHTML=`<div class='success-text-wrap'>
   <button class='modal-close-btn' data-modal-close aria-label='close modal button'>
        <svg class='modal-window-close-button' width='15' height='15'>
          <use href='${n}#icon-close-btn'></use>
        </svg>
  ...
  `}).catch(o=>{o.response.status===409?O.innerHTML=`<div class='text-wrap'>
   <button class='modal-close-btn' data-modal-close aria-label='close modal button'>
        <svg class='modal-window-close-button' width='15' height='15'>
          <use href='${n}#icon-close-btn'></use>
        </svg>
  ...
  `,window.addEventListener("keydown",e);function e(a){a.key==="Escape"&&f.classList.add("is-hidden")}window.onclick=function(a){a.target===f&&f.classList.add("is-hidden")};const t=document.querySelector(".modal-close-btn");t.addEventListener("click",s);function s(a){a.currentTarget===t&&f.classList.add("is-hidden")}}const k=document.querySelector(".list-product"),x=document.querySelector(".form-search"),D=document.querySelector(".error"),B=document.querySelector("#selected");D.style.display="none";let y,v;x.addEventListener("submit",X);function X(e){e.preventDefault();const{search:t}=e.currentTarget.elements;y=t.value,localStorage.setItem("SaveFilters",JSON.stringify(y)||null),M()}x.elements.search.value=localStorage.getItem("savetext");M();B.addEventListener("change",Z);function Z(e){e.target.value;const t=e.target.value;v=t,v=t,localStorage.setItem("SaveCategpries",JSON.stringify(v)||null),M()}x.elements.selecteds.value=localStorage.getItem("saveselected");function ee(){ae().then(e=>{const t=e.map(s=>`<option value="${s}">${String(s).replace("_"," ").replace("_"," ")}</option>`).join("");B.insertAdjacentHTML("beforeend",t)}).catch(e=>{console.log(e)})}ee();function M(){P().then(e=>{e.results.length===0&&(D.style.display="flex"),F(e.results)}).catch(e=>{throw new Error(e)})}function F(e){const t=e.map(({name:s,img:a,category:o,price:c,size:i,popularity:d,is10PercentOff:l,_id:u})=>l==!0?`
    ...
    </li>`).join("")}async function le(){const e=await oe(),t=e.reduce((a,o)=>(o.name.length>10?a.push(o.name.slice(0,10)+"..."):a.push(o.name),a),[]);H.innerHTML=de(e,t),H.addEventListener("click",a=>{const o=a.target.closest(".card_item");if(o){const c=o.dataset.id;E(c)}}),document.querySelectorAll(".basket").forEach(a=>{a.addEventListener("click",ue);const o=a.dataset.id;b(o)&&(a.setAttribute("disabled","true"),a.innerHTML=`
        <svg class="basket-icon-check" width="18" height="18">
          <use href="${n}#icon-check"></use>
        </svg>
      `)})}le();function ue(e){const t=e.currentTarget,s=t.dataset.id;b(s)?(_(s),t.removeAttribute(s),t.innerHTML=`
      <svg class="basket-icon" width="18" height="18">
        <use href="${n}#icon-cart"></use>
      </svg>
    `):(j(s),t.setAttribute("disabled",!0),t.innerHTML=`
      <svg class="basket-icon-check" width="18" height="18">
        <use href="${n}#icon-check"></use>
      </svg>
    ...
    </li>`).join("");t.innerHTML=s,W.appendChild(t)}function he(e){const t=e.target.closest(".product-content");if(t){const s=t.dataset.id;E(s)}}W.addEventListener("click",he);pe();document.querySelector(".pagination");let q=0;const me=window.innerWidth;me<=767?q=2:q=4;let T=1;function fe(e,t){const s={totalItems:e.totalPages*10,visiblePages:t,template:{page:'<a href="#" id="page-btn" class="tui-page-btn">{{page}}</a>',currentPage:'<strong id="is-selected" class="tui-page-btn tui-is-selected">{{page}}</strong>',moveButton:'<a href="#" id="arrow-btn" class="tui-page-btn tui-{{type}} custom-class-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></a>',disabledMoveButton:'<span id="arrow-btn" class="tui-page-btn tui-is-disabled tui-{{type}} custom-class-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></span>',moreButton:'<a href="#" id="page-btn" class="tui-page-btn tui-{{type}}-is-ellip custom-class-{{type}}"><span class="tui-ico-ellip">...</span></a>'}};new z("pagination",s).on("afterMove",function(o){T=o.page,P(T).then(c=>{F(c.results)}).catch(c=>{throw new Error(c)})})}P(T).then(e=>{fe(e,q)}).catch(e=>{throw new Error(e)});function ve(e){const t=e.currentTarget;console.log(t);const s=t.dataset.id;console.log(s),t.hasAttribute("disabled")?(removeFromStorageCart(s),console.log(removeFromStorageCart(s)),t.removeAttribute("disabled"),t.innerHTML=`
      <svg class="basket-icon" width="18" height="18">
    ...
    `)});export{ce as g,$ as h,ye as o,_ as r};
//# sourceMappingURL=pagination-1e857e97.js.map
